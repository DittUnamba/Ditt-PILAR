...
/*************************************************************************
 *   No need extern Libs - LxMailer 2022 (09)
 *   Re-coded by : Ramiro Pedro Laura Murillo
 *
 *         2014 : Based on CodeIgniter_Email engine 2014
 *   07-09-2022 : SMTP and Sendmail auto mode and inline config
 *
 **************************************************************************/
...
class MailerCI {
    ...
    // Default config to send
    //
	public $useragent	= 'CI_LxMailer';
    ...
    private $mode = "sendmail";
    private $sign = "MailerX";
    private $from = "pilar@unap.edu.pe";
	// --------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------
    // LxMailer code - no libs implementation for compatibilty
    // --------------------------------------------------------------------
    public function Sign( $arg ) { $this->sign = $arg; }
    ...
    public function Send( $arrMail, $subject, $msg, $brochFile="" )
    {
        ...
        $this->from( $this->from, $this->sign );
        $this->to( $arrMail[0] ); //"rlaura@unap.edu.pe" );
    ...
    public function SMTP( $user="", $pass="", $host="", $port="" )
    {
        ...
        $config['smtp_port'] = $port? $port : '465';
        $config['smtp_host'] = $host? $host : 'ssl://smtp.gmail.com';
        $config['smtp_user'] = $user? $user : 'pilar@unap.edu.pe'; //opid
        $config['smtp_pass'] = $pass? $pass : 'opidunap++2';
    ...
    // --------------------------------------------------------------------
    // Original CI code ==> only getInstance() was disabled
	// --------------------------------------------------------------------
	 ...
	 * @return	bool
	 */
	public function send_ci($auto_clear = TRUE)
	{
