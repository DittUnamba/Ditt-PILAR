...
class Tesistas extends CI_Controller {
    ...
    public function ver()
    {
        $mail = "primero@unamba.edu.pe";
        $tbl = $this->dbPilar->getSnapRow( "vxDatTesistas" );//, "Correo='$mail'" );
    ...
    public function demoMail()
    {
        ...
        //echo "sasa";
        $this->genmailer->mailPilar( "rplm.mx@gmail.com", "Prueba Pilar", "mensaje uno dos ultraviolento..." );
        //$this->genmailer->mailPilar( "rlaura@unap.edu.pe", "demo", "muuuuuu" );
    }
    ...
    */
    ...
    public function lineasTes()
    {
        ...
        $lineas = $this->dbRepo->getTable("tblLineas","Estado = '1'");
    ...
    public function tesProyecto()
    {
        ...
        //echo "Los proyectos son registrador en la Unidades y/o Direcciones de Investigación";
        //$this->load->view( "pilar/tesb/regproy" );
        //return;
    ...
    public function subirArchevo( $tipo=1, $kind="pdf" )
    {
        ...
        if( $kind=="pdf" ){
            $config['upload_path']   = getcwd().'/repositor/docs/';

        } else {
            $config['upload_path']   = getcwd().'/repositor/foto/';
        }
        ...
        if( $tipo == 3 ){
            $config['upload_path']   = getcwd().'/repositor/bach/';
    ...
    public function loadLinCbo( $tipjur, $linea )
    {
        ...
        if( $tipjur == 4 )
        {
			...
			*/
            ...
            $table = $this->dbPilar->getSnapView(
                "vxDocInLin",
                "IdCategoria<='9' AND Activo>=5 AND IdLinea='$linea' AND LinEstado = 2",
                "ORDER BY DatosPers" );
    ...
    public function jsBusqTes()
    {
        ...
        $data = $this->dbRepo->GetSnapRow( "unamba_matrics", " CODIGO = '$codigo' " );
        //print_r( $data ); // 47490383

        if( ! $data ){
            echo "<b> Datos no encontrados </b>";
            return;
        }

        if( $data->DNI != $numdni ){
            echo "<b> El documento de identidad no coincide </b>";
            return;
        }

        //
        // seteamos la data del formato que venga a una estructura general
        //
        mlSetGlobalVar( "proRec", array(
            'IdCarr'  => $data->IdEP,
            'IdFacu'  => 1,
            ...
            'DNI'     => "$data->DNI",
            'Codigo'  => "$data->CODIGO",
            'SemReg'  => "$data->SEMESTRE", //matricula->anio."-".$data->matricula->periodo." (".$data->matricula->semestre.")",
            'Nombres' => "$data->NOMBRES",
            'Apellis' => "$data->APPATERNO $data->APMATERNO",
        ) );

        // UPDATE `unamba_matrics` SET IdEP=1 WHERE EP='ADMINISTRACIÓN'
